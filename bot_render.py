"""
╔══════════════════════════════════════════╗
║      RENDER VERSION  (Webhook 24/7)      ║
║  Deploy this on Render for always-on     ║
...
# ── load_dotenv does nothing on Render (env vars set in dashboard) ─────────────
load_dotenv()

BOT_TOKEN      = os.getenv("BOT_TOKEN")
GROQ_API_KEY   = os.getenv("GROQ_API_KEY")
TAVILY_API_KEY = os.getenv("TAVILY_API_KEY")
WEBHOOK_URL    = os.getenv("WEBHOOK_URL")        # https://telegram-ai-bot-2.onrender.com
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    f"User name is {user_name}. Today is {TODAY}.\n\n"
                    f"RULES:\n"
                    f"1. If web results are provided, use ONLY that data\n"
...
async def clear_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_history[update.effective_user.id].clear()
...
# ── Main (WEBHOOK — required for Render) ──────────────────────────────────────
def main():
    print("=" * 45)
    print("   RENDER BOT STARTED  (Webhook mode)")
    print(f"   Today       : {TODAY}")
    print(f"   Port        : {PORT}")
    print(f"   Webhook URL : {WEBHOOK_URL}/{BOT_TOKEN}")
    print("=" * 45)

    # Crash early with clear message if any env var is missing
    ...
    # WEBHOOK — Telegram sends messages to your Render URL
    app.run_webhook(
        listen="0.0.0.0",
        port=PORT,
        url_path=BOT_TOKEN,
